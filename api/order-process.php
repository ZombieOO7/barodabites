...
if(isset($_POST['place_order']) && isset($_POST['user_id']) && !empty($_POST['product_variant_id'])){
    ...
    if (isset($_POST['promo_code']) && $_POST['promo_code'] != '') {
        ...
        $response=$function->validate_promo_code($user_id,$promo_code,$total,json_decode(stripslashes($_POST['product_variant_id'])));
        if($response['error']==true){
            echo json_encode($response);
            exit();
        }
        $discount = $response['discount'];
        $final_total = $response['discounted_amount'];
    }
    ...
    echo $wallet_used;
    exit;*/
    /*echo $user_wallet_balance;exit;
    if($wallet_used=='true' && $user_wallet_balance){
        echo 'first';exit;
    }*/
    $walletamt = '';
    if($user_wallet_balance != 0 && $wallet_used == 'true'){
        $walletamt = $wallet_balance;
        /* deduct the balance & set the wallet transaction */
        // $new_balance = ($user_wallet_balance - $wallet_balance);
        // $function->update_wallet_balance($new_balance,$user_id);
        /* add wallet transaction */
        //echo 'here111';exit;
        // if($user_wallet_balance <= $total){
            // $wallet_txn_id = $function->add_wallet_transaction($user_id,'debit',$walletamt,'Used against Order Placement');   
        // }else{
            // $wallet_txn_id = $function->add_wallet_transaction($user_id,'debit',$total,'Used against Order Placement');
        // }
    }else{
        //echo 'here';exit;
        $wallet_used = false;
    ...
    if($walletamt > 0 && $wallet_used == 'true')
    {
        $msg = 'Used against Order Placement (Order Id : '.$order_id.')';
        $wallet_txn_id = $function->add_wallet_transaction($user_id,'debit',$walletamt,$msg);   
    }
    
    //$function->update_wallet_transaction($order_id,$wallet_txn_id);   
    // print_r($item_details);
    for($i=0;$i<count($item_details);$i++){
        ...
        if($db->update('orders',$data,'id='.$order_id)){// Table name, column names and respective values
            ...
            for($i=0;$i<count($item_details);$i++){
                $product_id = $item_details[$i]['product_id'];
                array_push($arrayprod,$product_id);
                // $unit = (float) filter_var( $item_details[$i][2], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION ) ; // float(55.35) 
                $measurement = $item_details[$i]['measurement'];
                $product_variant_id = $item_details[$i]['id'];
                // echo $product_variant_id;
                ...
                $sub_total1 = $sub_total1 + $total_price;
                // print_r($item_details[0]['price']);
                $message .= "<b>Name : </b>".$item_details[$i]['name']."<b> QTY :</b>".$quantity."<b> Subtotal :</b>".$item_details[$i]['discounted_price']*$quantity."<br>";
                $message_1 .= "<tr>
                    <td>".$item_details[$i]['name']."</td>
                    <td class='text-right'>".$quantity."</td>
                    <td class='text-right'>".$discounted_price."</td>
                    <td class='text-right'>".$discounted_price*$quantity."</td>
                </tr>"; 
                $total_new = $total_new + ($discounted_price*$quantity);
            }
            $total_new_dc = $total_new + $delivery_charge;
            $promocodehtml = '';
            if($promo_code){
                if($response['free_product'] == ''){
                    $dtype = 'Discount';
                }else{
                    $dtype = 'Free Product';
                }
                $final_total_a = $total_new_dc - $response['discount'];
                $promocodehtml = '<tr>
                   <td class="text-right" colspan="3"><b>Promo Code</b></td>
                   <td class="text-right">'.$promo_code.'</td>
               </tr><tr>
                   <td class="text-right" colspan="3"><b>Discount</b></td>
                   <td class="text-right">'.$response['discount'].'</td>
                </tr><tr>
                   <td class="text-right" colspan="3"><b>Total</b></td>
                   <td class="text-right">'.$final_total_a.'</td>
                </tr>';
            }
            $message = "Hello, Dear ".ucwords($res[0]['name']).", We have received your order successfully. Your order is being processed. ";
            $message .= "Your Order ID is : #".$response['order_id']." Thank you for choosing BarodaBites App. Have a great day.";
...
//          $message .= "<br><br>Thank you for placing an order with us!<br><br>You will receive future updates on your order via Email!";
            ...
            $message .= '<div class="container">
                               <div style="page-break-after: always;border: 1px solid #ddd;">
                                      <h1 style="border-bottom: 3px solid #ddd;">Invoice #'.$order_id. '</h1>
                                  <table class="table table-bordered" style="border-bottom: 2px solid #ddd; width: 100%;border: 1px solid #ddd;padding: 10px;">
                                     <thead>
                                        <tr>
                                     ...
                                     <tbody>
                                        <tr>
                                           <td style="width: 50%;">
                                              <address>
                                                 <strong>Baroda Bites</strong><br>
                                                 6 - Jalaram Park,  Ajawa Road, Vadodara,<br>
                                                 Gujarat, India. - 390019<br>
                                                 GSTIN No. :- 24CLQPK9620l1ZL 
                                              </address>
                                              <b>Telephone</b> +91 8140002400<br>
                                              <b>E-Mail</b> Info@barodabites.com<br>
                                              <b>Web Site:</b> <a href="www.barodabites.com">https://www.barodabites.com</a>
                                           </td>
                                           <td style="width: 50%;"><b>Date Added</b> '.$data['date_added'].'<br>
                                        ...
                                        <tr>
                                           <td style="width: 50%;">
                                              <address>
                                                 ...
                                                 <b>Name : </b>'.ucwords($res[0]['name']).'<br>
                                                 <b>E-Mail:</b><br>
                                                 '.$res[0]['email'].'<br>
                                                 <b>Telephone:</b>'.$mobile.'                 </address>
                                           </td>
                                           <td style="width: 50%;">
                                              <address>
                                  ...
                                  <style>
                                      .pro td {border: 1px solid #ddd;}
                                  </style>
                                  <table style="width: 100%; border: 1px solid #ddd;" class="table table-bordered pro">
                                     <thead>
                                        <tr>
                                           <td><b>Product</b></td>
                                           <!--<td><b>Model</b></td>-->
                                     ...
                                     <tbody>
                                        ...
                                        <tr>
                                           ...
                                           <td class="text-right">'.$total_new.'</td>
                                       </tr>
                                       <tr>
                                        <td class="text-right" colspan="3"><b>Dilivery Charge</b></td>
            ...
            $subjectAdmin = "Baroda Bites - Order".$response['order_id'];
            send_email('barodabites@gmail.com',$subjectAdmin,$message);
...
if(isset($_POST['update_order_item_status']) && isset($_POST['order_item_id'])) {
    ...
    if(!empty($result)){
        ...
        if($postStatus == 'cancelled'){
            $sql = 'SELECT final_total,total,user_id,payment_method,delivery_charge FROM orders WHERE id='.$order_id;
            $db->sql($sql);
            ...
            if($res_oi[0]['discounted_price']==0){
               $updated_final_total = $res_order[0]['final_total']- ( $res_oi[0]['price'] * $res_oi[0]['quantity'] ); 
               $updated_total = $res_order[0]['total']-( $res_oi[0]['price'] * $res_oi[0]['quantity'] ); 
            }else{
               $updated_final_total = $res_order[0]['final_total']-($res_oi[0]['discounted_price'] * $res_oi[0]['quantity']); 
               $updated_total = $res_order[0]['total']-($res_oi[0]['discounted_price'] * $res_oi[0]['quantity']); 
            }
            $delivery_charge = ($updated_final_total<$config['min_amount'])?$config['delivery_charge']:0;
            $updated_final_total = ($updated_final_total<$config['min_amount'] && $res_order[0]['delivery_charge']==0 )?$updated_final_total+$delivery_charge:$updated_final_total;
            
            $sql_order = 'update orders set delivery_charge = '.$delivery_charge.',total='.$updated_total.',final_total='.$updated_final_total.' WHERE id='.$order_id;
            $db->sql($sql_order);
