...
if($access_key != $accesskey && $_POST['type']!='delete-notification'){
	$response['error']= true;
...
if ((isset($_POST['type'])) && ($_POST['type'] == 'verify-user')) {
	...
	if (!empty($mobile)) {
		...
		if($num_rows > 0){
			...
			$message = "<#> Your OTP for Apna Dukan Registration is : ".$otp." 7iFr/ICNwpV";
...
if(isset($_POST['type']) && $_POST['type'] != '' && $_POST['type'] == 'verify-user-email') {
	...
	if($num_rows == 0){
		//send email
		$to = $email;
		$subject = "BarodaBites Registration Verification";
		
		$message = "<#> Your OTP for BarodaBites Registration verification is : ".$otp.". Please enter this OTP to activate your profile. ";
		if($mobile){
		    sendSms($mobile, $message);   
		    $response["error"]   = false;
    		$response["message"] = "OTP is sent to your mobile number. Please verify it."/*.$smsResult*/;
    		...
    		$response["error"]   = false;
    		$response["message"] = "OTP for account activation is sent to your email. Please verify it to complete the registration process";
    		$response["OTP"] = $otp;
		}
...
if ((isset($_POST['type'])) && ($_POST['type'] == 'register')) {
	...
	if (!empty($email)) {
		...
		if($num_rows > 0){
			...
			//gwt wallet balance
		/*
			$sql_query_wallet = "SELECT sum( amount ) FROM `wallet_transactions` WHERE user_id=".$res[0];
                $wallet_final = $db->sql($sql_query_wallet); print_r($wallet_final);
                if ($result1 = mysqli_query($conn, $sql_query_wallet)) { 
  
                    while ($row1 = mysqli_fetch_row($result1)) {  
                        
                        if ($row1[0] > 0) {
                             $wallet_amount = $row1[0];
		...
		//	$response['wallet_balance'] = $wallet_amount;
...
if(isset($_POST['type']) && $_POST['type'] != '' && $_POST['type'] == 'forgot-password-mobile') {
	...
	if($db->numRows($result) > 0){
		//send sms
		$message = 'Your Password for Apna Dukan is Reset. Please login using new Password : '.$password.'.';
		sendSms($mobile, $message);
